...
def _apply_experience_level_filter(score: float, candidate_level: str, required_experience_level: str) -> tuple[float, str] | None:
    """Filters results based on the required experience level."""
    if not candidate_level or not required_experience_level:
        return score, None

    candidate_tier = _TIER_ORDER.get(candidate_level)
    required_tier = _TIER_ORDER.get(required_experience_level)
    if candidate_tier is None or required_tier is None:
        return score, None

    distance = abs(candidate_tier - required_tier)
    if distance == 0:
        return score, None
    elif distance == 1:
        cap = 0.6
    ...
    if score > cap:
        note = (f"Candidate experience level '{candidate_level}' does not match required level '{required_experience_level}'. ")
        return cap, note
    return score, None
...
class JobMatchAgent:
    ...
    def _parse_match(self, raw: str, document_id: str, required_job_type: str = None, required_experience_level: str = None) -> JobMatchResult:
        ...
        try:
            ...
            candidate_experience_level = data.get("candidate_experience_level") or None

            summary = str(data.get("summary", "")).strip() or raw[:200].strip()

            score, cap_note = _apply_experience_level_filter(score, candidate_experience_level, required_experience_level)
            if cap_note:
                summary = f"{summary} | {cap_note}"
                logger.info(f"Candidate {document_id} | {summary}")
    ...
    async def _invoke_with_retry(self, job_context: str, candidate_context: str):
        try:
    ...
    """
    Score a candidate based on their resume and the job description.
    """
    async def _score_candidate(self, job_description: str, document_id: str, experience_level: str = None, job_type: str = None) -> JobMatchResult | None:
        ...
        constraints = []
        if experience_level:
            constraints.append(f"REQUIRED experience level: {experience_level}")
        job_context = job_description
        if job_type:
            constraints.append(f"REQUIRED job type: {job_type}")
        ...
        async with self._semaphore:
            try:
                response = await self._invoke_with_retry(job_context, candidate_context)
        ...
        return self._parse_match(response.content,document_id,required_job_type=job_type,required_experience_level=experience_level)

    # Asynchronous Public API
    async def arun(self, job_description: str, top_k: int = None, experience_level: str = None, job_type: str = None) -> list[JobMatchResult]:
        ...
        results = await asyncio.gather(*(self._score_candidate(job_description, doc_id, experience_level, job_type) for doc_id in document_ids))
